...
const testData = [
  {
    ...
    description:
      "Vel voluptatem id repudiandae aut omnis. deleniti aliquam quia magnam eos. Sunt saepe nisi delectus.",
...
export default function App() {
  ...
  const [totalPages, setTotalPages] = useState(
    Math.ceil(testData.length / MAX_RESULTS_PER_PAGE) - 1
  );
  ...
  const onHandleSearch = () => {
    ...
    const newData = testData.filter((item) => {
      return (
    ...
    setData(newData);
    setCurrentPage(0);
    setTotalPages(Math.ceil(newData.length / MAX_RESULTS_PER_PAGE) - 1);
  };
